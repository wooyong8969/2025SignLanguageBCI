...
class DWTFeatureExtractor:
    def __init__(self, fs=125, wavelet='coif1', level=1):
        self.fs = fs
    ...
    def extract(self, eeg_data):
        ...
        bands = ['cA' + str(self.level)] + ['cD' + str(i) for i in range(self.level, 0, -1)]
        self.bands = bands
        time_features = {band: [] for band in bands}
        freq_features = {band: [] for band in bands}

        for ep in range(n_epochs):
            for ch in range(n_channels):
                signal = eeg_data[ep, ch, :]
                coeffs = pywt.wavedec(signal, wavelet=self.wavelet, level=self.level)
                coeffs_dict = dict(zip(bands, coeffs))
                for band in bands:
                    x = coeffs_dict[band]
