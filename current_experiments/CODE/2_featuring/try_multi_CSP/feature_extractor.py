...
class DWTFeatureExtractor:
    ...
    def extract_multiclass_csp_features(self, eeg_data, labels, n_components=6):
        eeg_data = eeg_data.astype(np.float64)
        le = LabelEncoder()
        y = le.fit_transform(labels)
        covmats = []
        for x in eeg_data:
            cov = np.cov(x)
            cov += 1e-6 * np.eye(cov.shape[0])  # 정규화로 양의 정부호 보장
            cov /= np.trace(cov)
            covmats.append(cov)
        covmats = np.stack(covmats)
        class_covs = [np.mean(covmats[y == c], axis=0) for c in np.unique(y)]
        composite_cov = mean_riemann(covmats, tol=1e-6, maxiter=100)

        eigvals, eigvecs = eigh(class_covs[0], composite_cov)
        ix = np.argsort(np.abs(eigvals))[::-1]
        filters_ = eigvecs[:, ix[:n_components]]

        features = []
        for x in eeg_data:
            projected = filters_.T @ x
            var = np.var(projected, axis=1)
            logvar = np.log(var / np.sum(var))
