...
mat_path = os.path.join(data_path, "experiment_001(1-8)_cleaned.mat")
label_path = os.path.join(data_path, "experiment_001(1-8)_labels.csv")
...
# ---------- 4. 저장 ---------- #
sio.savemat(os.path.join(data_path, "experiment_001(1-8)_train_cleaned.mat"), {
    "EEG_clean": {
        "data": X_train.astype(np.float32),
        "srate": srate
    }
})
sio.savemat(os.path.join(data_path, "experiment_001(1-8)_test_cleaned.mat"), {
    "EEG_clean": {
...
pd.DataFrame(y_train).to_csv(
    os.path.join(data_path, "experiment_001(1-8)_train_labels.csv"), index=False
)
pd.DataFrame(y_test).to_csv(
    os.path.join(data_path, "experiment_001(1-8)_test_labels.csv"), index=False
)
